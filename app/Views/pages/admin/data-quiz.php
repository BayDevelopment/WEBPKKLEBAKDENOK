...
<div class="container-fluid dashboard-modern">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 page-title mb-3"><i class="fa-solid fa-circle-question"></i> <?= esc($sub_judul ?? 'Data Quiz') ?></h1>
            ...
            <div class="card shadow mb-4 card-modern">
                ...
                <div class="table-toolbar">
                    <div class="ms-auto d-flex align-items-center gap-2">
                        <a href="<?= base_url('admin/quiz/create') ?>" class="btn btn-gradient rounded-pill py-2"><i class="fa-solid fa-file-circle-plus mr-1"></i> Tambah Kategori</a>
                    </div>
                </div>
                            ...
                            <table id="tblQuiz" class="table table-striped table-bordered nowrap table-modern" width="100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Thumbnail</th>
                                        <th class="no-export">Judul</th>
                                        <th>Slug</th>
                                        <th>Kategori</th>
                                        <th class="text-center">Deskripsi</th>
                                        <th class="text-center">Durasi/Mnt</th>
                                        <th>Status</th>
                                ...
                                <tbody>
                                    <?php $no = 1;
                                    foreach ($quizzes as $qwz): ?>
                                        <?php
                                        $isActive = strtolower(trim($qwz['status'] ?? '')) === 'active';
                                        $thumb = !empty($qwz['thumbnail']) ? base_url('assets/uploads/quiz/' . esc($qwz['thumbnail'])) : base_url('assets/img/icons-empty.png');
                                        ?>
                                        <tr>
                                            ...
                                            <td><?= esc($qwz['judul'] ?? '-') ?></td>
                                            <td><span class="pill"><i class="fa-solid fa-tag"></i><?= esc($qwz['slug'] ?? '-') ?></span></td>
                                            <td class="text-center nowrap"><?= esc($qwz['kategori'] ?? '-') ?></td>
                                            <td class="text-center nowrap"><?= (int)($qwz['durasi_menit'] ?? 0) ?> menit</td>
                                            <td>
                                                <span class="badge-soft <?= $isActive ? 'active' : 'inactive' ?>"><?= $isActive ? 'Active' : 'Inactive' ?></span>
                                            </td>
                                            <td class="nowrap"><?= function_exists('indo_date') ? indo_date($qwz['created_at'] ?? '') : esc(date('d M Y', strtotime($q['created_at'] ?? 'now'))) ?></td>
                                            <td class="no-export">
                                                <div class="d-inline-flex">
                                                    <a href="<?= base_url('admin/quiz/soal/' . (string)(esc($qwz['slug']) ?? 0) . '/tambah') ?>" class="btn-action mr-1" title="Tambah Soal - <?= esc($qwz['kategori']) ?>"><i class="fa-solid fa-file-circle-plus"></i></a>
                                                    <a href="<?= base_url('admin/quiz/detail/' . (int)(esc($qwz['id_quiz']) ?? 0)) ?>" class="btn-action mr-1" title="Detail"><i class="fas fa-eye"></i></a>
                                                    <a href="<?= base_url('admin/quiz/update/' . (int)(esc($qwz['id_quiz']) ?? 0)) ?>" class="btn-action mr-1" title="Edit"><i class="fas fa-edit"></i></a>
                                                    <button type="button"
                                                        onclick="confirmDeleteQuiz(<?= esc($qwz['id_quiz'] ?? 0, 'js') ?>)"
                                                        class="btn-action"
                                                        title="Hapus">
        ...
        <div class="col-xl-4 col-lg-5">
            <div class="chart-card">
                ...
                <div class="chart-wrap">
                    <canvas id="soalPerKategori"></canvas>
                </div>
...
<script>
    ...
    // chart pekategori
    document.addEventListener('DOMContentLoaded', function() {
        const ctx = document.getElementById('soalPerKategori');
        if (!ctx) return;
